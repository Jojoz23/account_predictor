#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Test Tangerine bank extractor on all statements"""
...
def main():
    ...
    if not tangerine_folder.exists():
        print(f"❌ Folder not found: {tangerine_folder}")
        return
    
    pdf_files = sorted(tangerine_folder.glob("*.pdf"))
    
    if not pdf_files:
        print(f"❌ No PDF files found in {tangerine_folder}")
        return
    
    print("="*80)
    print("TESTING ALL TANGERINE BANK STATEMENTS")
    print("="*80)
    ...
    for pdf_file in pdf_files:
        ...
        try:
            df, opening, closing, year = extract_tangerine_bank_statement(str(pdf_file))
