#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Test RBC Mastercard extractor on all statements"""
...
def main():
    rbc_mastercard_folder = Path("data/RBC Mastercard 2364 Jan 10, 2025 - Dec 10, 2025")
    
    if not rbc_mastercard_folder.exists():
        print(f"❌ Folder not found: {rbc_mastercard_folder}")
        return
    
    pdf_files = sorted(rbc_mastercard_folder.glob("*.pdf"))
    
    if not pdf_files:
        print(f"❌ No PDF files found in {rbc_mastercard_folder}")
        return
    
    print("="*80)
    print("TESTING ALL RBC MASTERCARD STATEMENTS")
    print("="*80)
    ...
    for pdf_file in pdf_files:
        ...
        try:
            df, opening, closing, year = extract_rbc_mastercard_statement(str(pdf_file))
